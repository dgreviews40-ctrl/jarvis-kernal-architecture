...
function Start-PiperTTS {
    ...
    try {
        $piperDir = Join-Path $PWD "Piper"
        $process = Start-Process -FilePath "cmd.exe" `
                                   -ArgumentList "/k `"cd /d `"$piperDir`" `& piper.exe --port 5000`"" `
                                   -PassThru `
                                   -WindowStyle Minimized
...
function Wait-ForViteServer {
    ...
    while ($attempts -lt $maxAttempts) {
        ...
        if ($attempts % 5 -eq 0) {
            $statusMsg = "Still waiting... ($attempts" + " of $maxAttempts seconds)"
            Write-JarvisLog $statusMsg -Level INFO
        }
    }
